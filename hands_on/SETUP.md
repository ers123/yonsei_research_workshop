...
> **슬라이드(.pptx)는 공개 repo 에 포함되지 않습니다.** Google Drive 로 별도 공유 — 링크는 강사(yohan.harmony@gmail.com) 에게 요청.
...
- 없어도 강사 제공 **AI Index Report 2026** (`hands_on/references/ai_index/`)로 실습 가능
...
> 관심 있는 분은 워크숍 **이후 집에서** 이 섹션 + `hands_on/prompts/track_c_연구조수팀.md` Part B(홈스터디 가이드)를 따라 해 보세요.
...
```
# /loop 예시 — 반복 처리 패턴
/loop data/ 폴더의 파일을 하나씩 읽고 output/summaries/에 저장해줘
```
...
│   └── track_c_홈스터디_완주예시.md ← 홈스터디 단계별 재현 예시
├── templates/
│   ├── CLAUDE.md.example        ← 맥락 문서 템플릿
│   ├── CLAUDE.md.filled_example ← 실제 채워진 예시
│   ├── AGENTS.md.example        ← 에이전트 행동 규칙 템플릿
│   └── A_vs_B_response_examples.md ← 맥락 유/무 응답 비교 예시
├── references/
│   ├── claude_code_퀵가이드.md   ← Claude Code 명령 퀵 레퍼런스
│   └── ai_index/                ← Track B 실습 자료 (PDF + 추출 백업 + Naive/Harness 예시)
...
**문의: yohan.harmony@gmail.com 또는 워크숍 당일 질문**
