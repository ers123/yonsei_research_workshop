...
| **32GB RAM** | Qwen 3.5 9B | Gemma 4 26B (MoE) | 추론 + 교차검증. 본 가이드 기준. |
...
| **Phi-4-mini** | 3.8B | ~4GB | 보통 | 함수 호출 강점. 가벼운 검증 전용. |
...
```bash
# 32GB 기준 권장 모델
ollama pull qwen3.5:9b          # 주력 (Scout + Writer)
ollama pull gemma4:26b           # 검증용 (Verifier) — 다른 계열
ollama pull nomic-embed-text     # 임베딩 (RAG용, 선택)
...
- 검증 모델: gemma4:26b (교차 검증 — 다른 계열 사용)
- 검증 규칙: Writer가 작성한 모든 인용은 Verifier가 별도로 확인
```
...
**3단계 — Verifier (교차 검증) ← 다른 모델 사용**

별도 터미널에서:
```bash
ollama run gemma4:26b-a4b-it-q4_K_M "아래 literature review에서:
